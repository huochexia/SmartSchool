...
interface GoodsManagerDao {
    ...
    //获取购物车中商品
    @Query("SELECT * FROM GoodsOfShoppingCart")
    fun getShoppingCartAllGoods(): Observable<MutableList<GoodsOfShoppingCart>>

    //通过编号查类别
    @Query("SELECT * FROM goodscategory WHERE code = :code")
    fun getCategory(code: String): Observable<GoodsCategory>

    //关联查询，查类别的时候同时得到它的商品.
    @Query("SELECT * FROM GoodsCategory")
    fun loadCategory(): Observable<MutableList<CategoryAndAllGoods>>


    //更新购物车商品的数量
    @Query("UPDATE  GoodsOfShoppingCart SET quantity = :orderQuantity WHERE goods_name = :name ")
    ...
    //加入购物车
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertShoppingCartGoods(goodsOfShoppingCart: GoodsOfShoppingCart): Completable
    ...
    @Update
    fun updateShoppingCart(goodsOfShoppingCart: GoodsOfShoppingCart): Completable
    ...
    //从购物车中删除商品
    @Delete
    fun moveShoppingCart(goodsOfShoppingCart: GoodsOfShoppingCart): Completable
