...
interface IRemoteShoppingCartDataSource : IRemoteDataSource {
    fun insertGoodsOfShoppingCart(orderItem: NewOrderItem): Completable
}

class RemoteShoppingCartDataSourceImpl(
    private val service: IShoppingCartServiceManager
) : IRemoteShoppingCartDataSource {
    override fun insertGoodsOfShoppingCart(orderItem: NewOrderItem): Completable {
        return service.addShoppingCart(orderItem)
    }
